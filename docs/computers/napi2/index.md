---
sidebar_position: 5
sidebar_label: "* SBC NAPI2 (RK3568J)"
title: Одноплатный компьютер NAPI2
description: NAPI2 - компактный промышленный одноплатный компьютер на базе RK3568J. 2xEthernet 1Гбит, RS485, CAN, GPIO, LVDS, USB 3.0. Для автоматизации, IoT-шлюзов и встраиваемых решений.
keywords: [NAPI2, одноплатный компьютер, SBC, RK3568J, промышленный компьютер, IoT шлюз, RS485, CAN, Ethernet, LVDS, GPIO, Comintech, NapiWorld, встраиваемые системы, промышленная автоматизация]
---

# * Одноплатный компьютер NAPI2

:fire: **Новый одноплатный компьютер NAPI2** :fire:

Разработан для:

- промышленной автоматизации
- IoT-шлюзов
- систем сбора данных
- встраиваемых решений
- создания промышленных компьютеров
<!-- ![Внешний вид NAPI2 - передняя панель](img-nature/napi-2-mpt-soon.jpg) \ -->
![alt text](img-bra/napi2-front3.png)

## Основные преимущества

- Все интерфейсы на передней панели
- RS485 (встроен в плату), CAN, SPI, I2C, USB
- 2хEthernet 1Гбит
- Компактный размер (109.4х70.2)
- 2 колодки GPIO 2.0 мм для удобного встраивания в мезонин
- LVDS для промышленных панелей
- Документы поданы в комиссию Минпромторг


![Виды Napi2](img-mark/napi2-3mix.jpg) \
*NAPI2 - вид спереди, вид сверху, вид снизу*


## Все характеристики

![Схема расположения интерфейсов NAPI2](img-scheme/napi2-1000px.png) \
*NAPI2 - расположение и названия интерфейсов*

- Процессор: RK3568J (промышленный температурный диапазон)

Память

- Память ОЗУ: 4Гб
- Память ПЗУ: 32Гб (EMMC)
- Дополнительный слот памяти: SD

Интерфейсы

- 2xEthernet 1Гбит\с
- USB 3.0
- USB 2.0
- USB type-c otg\питание

Видео

- HDMI
- LVDS

## Разъемы на плате

- Питание 5В
- LVDS
- RTC (BAT)

![Разъемы на плате NAPI2](img-scheme/napi2-onboard.jpg) \
*Расположение разъемов на плате NAPI2*

## Карта GPIO

- Питание 5В (IN\OUT), 3В (OUT)
- Консоль (UART2)
- 3xUART, CAN, 2xSPI, 2xI2C
- USB 2.0

![Карта GPIO NAPI2](img-gpiomap/gpio-map%20-01.jpg) \
*Подробная карта GPIO с распиновкой разъемов*

Скачать в формате [PDF](pdf/napi2-gpio10.pdf)

## Сборочный чертеж и размеры

Скачать в формате [PDF](pdf/%D0%A1%D0%B1%D0%BE%D1%80%D0%BE%D1%87%D0%BD%D1%8B%D0%B9_%D1%87%D0%B5%D1%80%D1%82%D0%B5%D0%B6_NAPI_2_%D0%A2%D0%A4%D0%9F%D0%9C_469535_100_%D0%A1%D0%91.PDF)

### Размеры платы

![](img-scheme/napi2-dim.jpeg)

### Функциональная схема

![](img-scheme/napi2-func-scheme.jpeg)

## Дополнительные платы. Плата для прототипирования.

### Плата для отладки и прототипирования
- Питание 9-36
- Консоль
- Переходные колодки с шага 2.0 мм на шаг 2.54 мм

![Плата расширения для NAPI2](img-scheme/napi2-extboard.jpg) \
*Все интерфейсы доступны на колодке 2.54" и подписаны, надежное питание и консоль*

![alt text](img-bra/napi2-carrier.png)
*Плата для прототипирования с расширенным питанием и переходными колодками*

### Плата питания и АСУТП для FCU3568

![](img-extra-board/napi2-extra-board.jpg) \



## Программное обеспечение

- Armbian.
  - Vendor kernel (Recomended): https://download.napilinux.ru/linuximg/napi2/armbian-ditrib/vendor
  - Current kernel: https://download.napilinux.ru/linuximg/napi2/armbian-ditrib/current

- NapiLinux. https://download.napilinux.ru/napilinux/0.2.6.1/napilinux-napi-2-dev/

>:warning: **Все прошивки для плат NAPI в разделе ["Загрузки"](/downloads)**


> :boom: **[Заявка на тестирование](https://forms.yandex.ru/u/6985e2fe902902fb8edcd083)**:boom:
